...
class GRUCell(nn.Module):
    def __init__(self, d_in, num_units, n_nodes, concat_h=False, activation='tanh'):
        ...
        mlpnn_channel = d_in*n_nodes+num_units if concat_h else d_in*n_nodes
        self.forget_gate = MLPNN(c_in=mlpnn_channel, c_out=num_units, concat_h=concat_h)
        self.update_gate = MLPNN(c_in=mlpnn_channel, c_out=num_units, concat_h=concat_h)
        self.c_gate = MLPNN(c_in=mlpnn_channel, c_out=num_units, concat_h=concat_h)
...
class Tem_Cau_Decoder(nn.Module):
    def __init__(self, n_nodes,in_ch=1,hidden_ch=32,n_layers=1,shared_weights_decoder=False,concat_h=False):
        super().__init__()
        self.in_ch = in_ch
        ...
        if shared_weights_decoder:
            ...
            self.decoder = nn.Sequential(
                LocalMLP(in_channels=2*hidden_ch, out_channels=in_ch, kernel_size=1, n_nodes=n_nodes),
            )
        self.act = nn.LeakyReLU()
        self.cells = nn.ModuleList()
        for i in range(self.n_layers):
            self.cells.append(GRUCell(d_in=in_ch if i==0 else hidden_ch, num_units=hidden_ch, n_nodes=n_nodes,concat_h=concat_h))
